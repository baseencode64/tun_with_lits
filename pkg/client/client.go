...
func (c *Client) setupTunnel() (*tun.Interface, error) {
	...
	// Add DNS routes to ensure DNS resolution works through TUN
	c.cfg.Logger.Debug("Adding DNS routes for TUN interface", "dns_routes_count", len(DNSRoutesToTUN))
	if err = c.routes.Add(route.Opts{IfName: ifc.Name(), Routes: DNSRoutesToTUN}); err != nil {
		c.cfg.Logger.Warn("Failed to add DNS routes", "error", err)
		// Don't fail completely - main routes are more important
	}
	...
	if c.cfg.EnableIPv6 {
		...
		// Add IPv6 DNS routes
		c.cfg.Logger.Debug("Adding IPv6 DNS routes for TUN interface", "dns_routes_count", len(DNSRoutesToTUNIPv6))
		if dnsErr := c.routes.Add(route.Opts{IfName: ifc.Name(), Routes: DNSRoutesToTUNIPv6}); dnsErr != nil {
			c.cfg.Logger.Warn("Failed to add IPv6 DNS routes", "error", dnsErr)
			// Continue anyway - IPv6 DNS is less critical
		}
